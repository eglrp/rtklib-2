...
static void Record_Ambiguity(rtk_t *rtk, double *ep, int *week, double *sow)
{
	...
	int i = 0;
	int j = IT(&rtk->opt) + 1;
	p+=sprintf(p, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%04d%s%9.2f%s",
		(int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep, *week, sep, *sow, sep);
	...
	for (i = 0; i < MAXSAT; i++)
	{
		...
		p+=sprintf(p, "%10.4f%s", rtk->x[i + j], sep);
	}
	p+=sprintf(p,"\n");
	n = p - (char *)buff;
	if (n > 0 && outFp[OFILE_AMBIGUITY])  fwrite(buff, n, 1, outFp[OFILE_AMBIGUITY]);
}
...
static void Record_res(rtk_t *rtk, double *ep, char *sep, int *week, double *sow)
{
	...
	int i = 0;
	double resc, resp;
	p1+=sprintf(p1, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%04d%s%9.2f%s",
		(int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep, *week, sep, *sow, sep);
	p2+=sprintf(p2, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%04d%s%9.2f%s",
		(int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep, *week, sep, *sow, sep);
	...
	for (i = 0; i < MAXSAT; i++) {
		...
		p1+=sprintf(p1, "%10.4f%s", resc, sep);
		p2+=sprintf(p2, "%10.4f%s", resp, sep);
	...
	n1 = p1 - (char *)buff1;
	n2 = p2 - (char *)buff2;
	if (n1 > 0 && outFp[OFILE_RESC])  fwrite(buff1, n1, 1, outFp[OFILE_RESC]);
	if (n2 > 0 && outFp[OFILE_RESP])  fwrite(buff1, n2, 1, outFp[OFILE_RESP]);
}
...
static void update_stat(rtk_t *rtk, const obsd_t *obs, int n, int nv, int stat)
{
	...
	for (i = 0; i<3; i++)
		denu[i] = dxyz[i] = rtk->sol.rr[i] - ture_xyz[i];
	...
	p += sprintf(p, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%04d%s%9.2f%s%14.7f%s%14.7f%s%14.7f%s%7.4f%s%7.4f%s%7.4f%s%7.4f%s%02d%s%02d%s%4.2f%s%3.2f\n",
	...
	int nsize = p - (char *)buff;

	if (nsize>0&& outFp[OFILE_PPPPOS]) {
		fwrite(buff, nsize, 1, outFp[OFILE_PPPPOS]);
	}
    p = p- nsize;
	nsize = 0;
	char id[MAXSAT];
	...
	for (i = 0; i < MAXSAT; i++) {
		...
		int sat = i + 1;
		satno2id(sat, id);
		p += sprintf(p, "%3d %15.4f %15.4f %15.4f %15.4f %15.4f %15.4f %15.4f %15.4f %15.4f"
		...
		p += sprintf(p, "%15.4lf %15.4lf %4d", rtk->ssat[sat - 1].resp[0], rtk->ssat[sat - 1].resc[0], rtk->ssat[sat - 1].slip[0]);
		p += sprintf(p, "\n");
	}
	nsize = p - (char *)buff;
	if (nsize>0 && outFp[OFILE_PPP]) {
		fwrite(buff, nsize, 1, outFp[OFILE_PPP]);
	}
