...
#pragma comment(lib, "Ws2_32.lib")  //2017-1-15 fuq
...
const prcopt_t prcopt_default = { /* defaults processing options */
	PMODE_PPP_KINEMA,0,2, SYS_GPS|SYS_CMP ,   /* mode,soltype,nf,navsys */   /* positioning mode (PMODE_???) */ /* positioning mode (PMODE_???) */ /* number of frequencies (1:L1,2:L1+L2,3:L1+L2+L5) */ /* navigation system */
	//PMODE_PPP_KINEMA,0,2,SYS_GPS|SYS_CMP|,   /* mode,soltype,nf,navsys */ 
	GNSISB_CT,
	7.0*D2R,{{0,0}},           /* elmin,snrmask */      /* elevation mask angle (rad) *//* SNR mask */
	1,4,0,0,                    /* sateph,modear,glomodear,bdsmodear *///ck     /* satellite ephemeris/clock (EPHOPT_???) */  /* AR mode (0:off,1:continuous,2:instantaneous,3:fix and hold,4:ppp-ar) */ /* GLONASS AR mode (0:off,1:on,2:auto cal,3:ext cal) */ /* BeiDou AR mode (0:off,1:on) */
	5,0,10,1,                   /* maxout,minlock,minfix,armaxiter */       /* obs outage count to reset bias */   /* min lock count to fix ambiguity */ /* min fix count to hold ambiguity */    /* max iteration to resolve ambiguity */
	3,3,0,1,                    /* estion,esttrop,dynamics,tidecorr */   /* ionosphere option (IONOOPT_???) */      /* troposphere option (TROPOPT_???) */  /* dynamics model (0:none,1:velociy,2:accel) */  //	int tidecorr;       /* earth tide correction (0:off,1:solid,2:solid+otl+pole) */
	//1,1,0,1,
	1,0,0,0,0,                  /* niter,codesmooth,intpref,sbascorr,sbassatsel */  /* number of filter iteration */  /* code smoothing window size (0:none) */   /* interpolate reference obs (for post mission) */      /* SBAS correction options */  /* SBAS satellite selection (0:all) */
	0,4,                        /* rovpos,refpos */        /* rover position for fixed mode */  /* base position for relative mode (0:pos in prcopt,  1 : average of single pos,2:read from file, 3:rinex header, 4:rtcm pos) */
	{100.0,100.0},              /* eratio[] */  /* code/phase error ratio */    /* code/phase error ratio */
	{100.0,0.003,0.003,0.0,1.0}, /* err[] */    /* measurement error factor */
	{30.0,0.03,0.3},            /* std[] */       /* initial-state std [0]bias,[1]iono [2]trop */
	//update
	{1E-7,1.5E-3,1E-4,1E-1,1E-2,0.0}, /* prn[] */    /* process-noise std [0]bias,[1]iono [2]trop [3]acch [4]accv [5] pos */
	5E-12,                      /* sclkstab */       /* satellite clock stability (sec/sec) */
	{5.0,0.9999,0.25,0.1,0.05}, /* thresar */        /* AR validation threshold */
	0.0,0.0,0.05,               /* elmaskar,almaskhold,thresslip */      /* elevation mask of AR for rising satellite (deg) */   /* elevation mask to hold ambiguity (deg) */ /* slip threshold of geometry-free phase (m) */
	30.0,30.0,30.0,             /* maxtdif,maxinno,maxgdop */            /* max difference of time (sec) */  /* reject threshold of innovation (m) */   /* reject threshold of gdop */
	{ 0 },{ 0 },{ -2005046.4452 ,5411158.3826 ,2707856.6022 },           /* baseline,ru,rb */   /* rover position for fixed mode {x,y,z} (ecef) (m) */   /* base position for relative mode {x,y,z} (ecef) (m) */
...
const solopt_t solopt_default={ /* defaults solution output options */
    ...
    0,1,1,0,0,0,                /* degf,outhead,outopt,datum,height,geoid */
...
*          matirix stored by column-major order (fortran convention)                       //for examble A=12*7   Y=12*1           Y=1*12  AY=A'*Y'=7*1
*-----------------------------------------------------------------------------*/
               ...
               double *Q)
{
    ...
    if (m<n) return -1;
    Ay=mat(n,1);
    matmul("NN",n,1,m,1.0,A,y,0.0,Ay); /* Ay=A*y */   //input a=7*12 y=12*1  NN:a*y  T表示*转置
    matmul("NT",n,n,m,1.0,A,A,0.0,Q);  /* Q=A*A' */   //input a=7*12 a=7*12  NT:a*a'
	...
	double *xp, double *Pp)
{
	...
	matmul("NN", n, m, n, 1.0, P, H, 0.0, F);       //F=P*H
													//F(n,m),P(n,n),H(n,m)
	matmul("TN", m, m, n, 1.0, H, F, 1.0, Q);       //Q=H'*F+R            /* Q=H'*P*H+R */
													//Q(m,m),H(n,m),F(n,m)
													//if (!(info=matinv(Q,m))) {
...
extern int filter(double *x, double *P, const double *H, const double *v, double *R, int n, int m)
{
	...
	double sum_deltx = 0.0;
	ix = imat(n, 1);
	for (i = k = 0; i < n; i++)
		if (x[i] != 0.0&&P[i + i * n] > 0.0) ix[k++] = i;//找出状态方程未知数所在列数,K未知参数个数=位置+钟差+tro+ion+卫星个数

	x_ = mat(k, 1); xp_ = mat(k, 1); P_ = mat(k, k); Pp_ = mat(k, k); H_ = mat(k, m);
	for (i = 0; i < k; i++) {
		x_[i] = x[ix[i]];
		for (j = 0; j < k; j++) P_[i + j * k] = P[ix[i] + ix[j] * n];//155*155->n*n 为
...
extern int readpcv(const char *file, pcvs_t *pcvs)
{
  ...
  //  
  //  if (!(ext=strrchr(file,'.'))) //strrchr() 将会找出 str 字符串中最后一次出现的字符 c 的地址，然后将该地址返回
		//ext="";
  //  
...
extern void getGPT(const double *pos, double dmjd, double *pres, double *temp, double *undu)
{
	...
	const double TWOPI_rtk = 6.283185307179586476925287;
	...
	PRES0 = APM + APA * cos(DOY / 365.25*TWOPI_rtk);
	...
	TEMP0 = ATM + ATA * cos(DOY / 365.25*TWOPI_rtk);
	...
	double humi, double *zwd, int atmodel)
{
	...
	if (pos[2]<-100.0 || pos[2]>1E6 || azel[1] <= 0) { return 0; }
...
{
    int i=(int)(lat/15.0);//小于15°
    if (i<1) 
		return coef[0];   
	else if (i>4)        //大于75°
		return coef[4];
    return 
		coef[i-1]*(1.0-lat/15.0+i)+coef[i]*(lat/15.0-i); //内插
	   // coef[i - 1] + (coef[i] - coef[i - 1])/(lat / 15.0 - i);
		}
...
static double interpvar0(int sat, double ang, const double *var, int bsat)
{
	...
	if (bsat) {
		...
		if (ang >= limit) {
			if (ang>limit + 0.25) {
				//sprintf(PPP_Glo.chMsg, "%s (nadir=%f) >= %2d°\n", PPP_Glo.sFlag[sat - 1].id, ang, limit);
			//	outDebug(OUTWIN, OUTFIL, 0);
			}
			return var[limit];
		}
