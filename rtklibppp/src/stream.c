...
static ntrip_t *openntrip(const char *path, int type, char *msg)
{
    ...
    printf("openntrip: path=%s type=%d\n",path,type);
	...
	printf("%s  %s  %s  %s  %s  %s", addr, port, ntrip->user, ntrip->passwd, ntrip->mntpnt,
		ntrip->str);
    /* use default port if no port specified */
    if (!*port) {
        printf("%d",NTRIP_CLI_PORT);
    }
    printf("%s:%s",addr,port);
    
    /* ntrip access via proxy server */
    if (*proxyaddr) {
        printf(ntrip->url,"http://%s",tpath);
        strcpy(tpath,proxyaddr);
    }
    /* open tcp client stream */
    if (!(ntrip->tcp=opentcpcli(path,msg))) {
...
{
    printf("stropen: type=%d mode=%d path=%s\n",type,mode,path);
