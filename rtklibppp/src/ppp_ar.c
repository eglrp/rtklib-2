                       ...
                       const nav_t *nav, const double *azel)
{
    ...
    for (i=0;i<n;i++) {
        ...
        EW=lamE*(obs[i].L[1]-obs[i].L[2])- lamN*(obs[i].P[1]/lam[1]+obs[i].P[2]/lam[2]);
        var=SQR(0.7*rtk->opt.eratio[0])* (SQR(rtk->opt.err[1])+SQR(rtk->opt.err[2]/sin(azel[1+2*i])));
                      ...
                      const nav_t *nav, const double *azel)
{
    ...
    for (i=0;i<n;i++) {
        ...
        MW=lamW*(obs[i].L[0]-obs[i].L[l])- lamN*(obs[i].P[0]/lam[0]+obs[i].P[l]/lam[l]);   //WM组合 相位宽带-伪距窄带
        var=SQR(0.7*rtk->opt.eratio[0])*  (SQR(rtk->opt.err[1])+SQR(rtk->opt.err[2]/sin(azel[1+2*i])));
                        ...
                        double *P)
{
    ...
    for (i=0;i<ns;i++) {
        ...
        Bw=(amb1->LC[0]-amb2->LC[0])/lamW;  //星间单差
        Bw+=nav->wlbias[sat1[i]-1]-nav->wlbias[sat2[i]-1]; /* correct WL-bias *星间单差-WL(s,s0)*/
        Nw=ROUND(Bw);
        B1=(x[j]-x[k]+C2*lam[l]*Nw)/lamN;  //窄带模糊度
        N1=ROUND(B1);
