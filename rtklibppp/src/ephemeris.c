...
static double var_urassr(int ura,int sys)
{
    double std;
	if (sys == SYS_CMP)
	{
		if (ura <= 0) return SQR(DEFURASSR_BDS);
                    ...
                    double *var)
{
	...
	//fprintf(fp_debug, "eph2pos : time=%s sat=%2d\n",time_str(time,3),eph->sat);
    ...
    if (n>=MAX_ITER_KEPLER) {
		...
		fprintf(outFp[OFILE_DEBUG], "kepler iteration overflow sat=%2d\n",eph->sat);
        return;
    }
    sinE=sin(E); cosE=cos(E);
    
	//fprintf(fp_debug, "kepler: sat=%2d e=%8.5f n=%2d del=%10.3e\n",eph->sat,eph->e,n,E-Ek);
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    if (iode>=0||j<0) {
		//fprintf(fp_debug, "no broadcast ephemeris: %s sat=%2d iode=%3d\n",time_str(time,0),
            //  sat,iode);
        return NULL;
    }
    return nav->eph+j;
                  ...
                  int iode, double *rs, double *dts, double *var, int *svh)
{
    ...
    *svh=-1;
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP) {
        ...
        eph2pos(time,eph,rs,dts,var);//º∆À„Œ¿–«Œª÷√
                      ...
                      int opt, double *rs, double *dts, double *var, int *svh)
{
	...
	//fprintf(fp_debug, "satpos_ssr: time=%s sat=%2d\n",time_str(time,3),sat);
    
    ssr=nav->ssr+sat-1;
    
    if (!ssr->t0[0].time) {
		if(outFp[OFILE_DEBUG])
		fprintf(outFp[OFILE_DEBUG], "no ssr orbit correction: %s sat=%2d\n",time_str(time,0),sat);
        return 0;
    }
    if (!ssr->t0[1].time) {
		if (outFp[OFILE_DEBUG])
		fprintf(outFp[OFILE_DEBUG], "no ssr clock correction: %s sat=%2d\n",time_str(time,0),sat);
        return 0;
    }
    /* inconsistency between orbit and clock correction */
    if (ssr->iod[0]!=ssr->iod[1]) {
		if (outFp[OFILE_DEBUG])
		fprintf(outFp[OFILE_DEBUG], "inconsist ssr correction: %s sat=%2d iod=%d %d\n",
    ...
    if (fabs(t1)>MAXAGESSR||fabs(t2)>MAXAGESSR) {
		if (outFp[OFILE_DEBUG])
		fprintf(outFp[OFILE_DEBUG], "age of ssr error: %s sat=%2d t=%.0f %.0f\n",time_str(time,0),
              sat,t1,t2);
        *svh=-1;
        return 0;
    }
    ...
    if (norm(deph,3)>MAXECORSSR||fabs(dclk)>MAXCCORSSR) {
		if (outFp[OFILE_DEBUG])
		fprintf(outFp[OFILE_DEBUG], "invalid ssr correction: %s deph=%.1f dclk=%.1f\n",
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        if (j>=NFREQ) {
			//fprintf(fp_debug, "no pseudorange %s sat=%2d\n",time_str(obs[i].time,3),obs[i].sat);
            continue;
        }
        ...
        if (!ephclk(time[i],teph,obs[i].sat,nav,&dt)) {
			if (outFp[OFILE_DEBUG])
			fprintf(outFp[OFILE_DEBUG], "no broadcast clock %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
			//printf("no broadcast clock %s sat=%2d\n", time_str(time[i], 3), obs[i].sat);
            continue;
        }
        time[i]=timeadd(time[i],-dt);
        
        /* satellite position and clock at transmission time */
        if (!satpos(time[i],teph,obs[i].sat,ephopt,nav,rs+i*6,dts+i*2,var+i,  //rs Œ¿–«Œª÷√∫ÕÀŸ∂» dts ÷”≤Ó∫Õ÷”∆Ø
                    svh+i)) {
			if (outFp[OFILE_DEBUG])
			fprintf(outFp[OFILE_DEBUG], "no ephemeris %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
		...
		if (ephopt == 0)
		{
		/*	fprintf(fp_signle_satpos, "%s sat=%2d rs=%13.3f %13.3f %13.3f dts=%12.3f var=%7.10lf svh=%02X\n",
				time_str(time[i], 0), obs[i].sat, rs[i * 6], rs[1 + i * 6], rs[2 + i * 6],
				dts[i * 2] * 1E9, var[i], svh[i]);*/
		    if (outFp[OFILE_SATPOS])
			fprintf(outFp[OFILE_SATPOS], "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%04d%s%9.2f%s%03d%s%13.3f%s%13.3f%s%13.3f%s%12.3f%s%10.7lf%s%02d\n",
				(int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep,week,sep, sow, sep,obs[i].sat, sep, rs[i * 6], sep, rs[1 + i * 6], sep, rs[2 + i * 6], sep,
				dts[i * 2] * 1E8,sep, var[i],sep, svh[i]);
		}
		if (ephopt == 1)
		{
			if (outFp[OFILE_PRECISE_SATPOS])
			fprintf(outFp[OFILE_PRECISE_SATPOS], "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%04d%s%9.2f%s%03d%s%13.3f%s%13.3f%s%13.3f%s%12.3f%s%10.7lf%s%02d\n",
				(int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep, week, sep, sow, sep, obs[i].sat, sep, rs[i * 6], sep, rs[1 + i * 6], sep, rs[2 + i * 6], sep,
				dts[i * 2] * 1E8, sep, var[i], sep, svh[i]);
		}
		if (ephopt == 3 || ephopt == 4)
		{
			if(outFp[OFILE_SSR_SATPOS])
			fprintf(outFp[OFILE_SSR_SATPOS], "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%04d%s%9.2f%s%03d%s%13.3f%s%13.3f%s%13.3f%s%12.3f%s%10.7lf%s%02d\n",
				(int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep, week, sep, sow, sep, obs[i].sat, sep, rs[i * 6], sep, rs[1 + i * 6], sep, rs[2 + i * 6], sep,
				dts[i * 2] * 1E8, sep, var[i], sep, svh[i]);
		}
    }
