...
/****************************ADD*************************/
    char	memwatchFile[50];
    //sprintf(memwatchFile, "%s/log/Der_MemWat.log", getenv("HOME"));
	sprintf(memwatchFile,  "libmem.log");
/****************************ADD*************************/
    ...
    if( mwLogR() == NULL ) mwLogFile(memwatchFile);
    if( mwLogR() == NULL ) {
        int i;
        char buf[32];
        ...
        for( i=1; i<100; i++ ) {
            ...
            sprintf( buf, "%s/log/MemWat%02d.log", getenv("HOME"), i );
            mwLogFile( buf );
