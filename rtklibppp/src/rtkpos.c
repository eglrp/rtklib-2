                 ...
                 double *e, double *azel)
{
    ...
    for (i=0;i<n;i++) {
        ...
        antmodel(1,opt->pcvr+index,opt->antdel[index],azel+i*2,opt->posopt[1],
                 dant);
...
//------------------------Calculation MP1 MP2 and tec---------------------

static int cal_MP(rtk_t *rtk, obsd_t *obs, int n, const nav_t *nav)
{
	double lam11, lam22, I, C, a;
	int  i,j,sat, sys;
	//double Mp[127][3];
	double temp1, temp2,temp3, deltMp1, deltMp2, deltTEC1;
	...
	for (i = 0; i < n; i++)
	{
		sat = obs[i].sat;
		//Mp[j][i] = 0.0;
		lam11 = nav->lam[sat - 1][0];
		lam22 = nav->lam[sat - 1][1];
		if (obs[i].P[0] * obs[i].P[1] * obs[i].L[0] * obs[i].L[1] == 0.0) continue;

		if (rtk->ssat[obs[i].sat - 1].obs0[sat - 1][0] * rtk->ssat[obs[i].sat - 1].obs0[sat - 1][1] *
			rtk->ssat[obs[i].sat - 1].obs0[sat - 1][2] * rtk->ssat[obs[i].sat - 1].obs0[sat - 1][3] == 0.0)
			continue;

		a = SQR(lam22) / SQR(lam11);
		C = 1.0 / (a - 1);
		rtk->ssat[obs[i].sat - 1].Mp1[obs[i].sat - 1][1] = obs[i].P[0] - (1 + 2 * a)*lam11*obs[i].L[0] + 2 * a*lam22* obs[i].L[1];//mp1
		rtk->ssat[obs[i].sat - 1].Mp2[obs[i].sat - 1][1] = obs[i].P[1] - 2 * a*C*lam11*obs[i].L[0] + (2 * a*C - 1)*lam22* obs[i].L[1];//mp2

		temp1 = rtk->ssat[obs[i].sat - 1].Mp1[obs[i].sat - 1][1];
		temp2 = rtk->ssat[obs[i].sat - 1].Mp2[obs[i].sat - 1][1];

		deltMp1 = rtk->ssat[obs[i].sat - 1].Mp1[obs[i].sat - 1][1] - rtk->ssat[obs[i].sat - 1].Mp1[obs[i].sat - 1][0];//mp12-mp11
		deltMp2 = rtk->ssat[obs[i].sat - 1].Mp2[obs[i].sat - 1][1] - rtk->ssat[obs[i].sat - 1].Mp2[obs[i].sat - 1][0];

		rtk->ssat[obs[i].sat - 1].Mp1[obs[i].sat - 1][0] = temp1;
		rtk->ssat[obs[i].sat - 1].Mp2[obs[i].sat - 1][0] = temp2;



		rtk->ssat[obs[i].sat - 1].TEC1[obs[i].sat - 1][1] = a * (lam11*obs[i].L[0] - lam22 * obs[i].L[1]);

		temp3 = rtk->ssat[obs[i].sat - 1].TEC1[obs[i].sat - 1][1];

		deltTEC1 = rtk->ssat[obs[i].sat - 1].TEC1[obs[i].sat - 1][1] - rtk->ssat[obs[i].sat - 1].TEC1[obs[i].sat - 1][0];//tec11-tec12

		rtk->ssat[obs[i].sat - 1].TEC1[obs[i].sat - 1][0] = temp3;

		
		int week;
		int tow;
		tow = time2gpst(rtk->sol.time, &week);
		if (tow == 351901) continue;
		if (sat == 1) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC1.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
			//	fprintf(fp, "%-10d		%-10d			%10.3lf			%10.3lf			%10.3lf\n", tow, sat, deltMp1, deltMp2, deltTEC1);
			fprintf(fp, "%-10d		%-10d			%10.3lf			%10.3lf			%10.3lf\n", tow, sat, deltMp1, deltMp2, deltTEC1);
			fclose(fp);
		}
		if (sat == 2) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC2.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 3) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC3.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 4) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC4.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 5) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC5.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 6) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC6.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 7) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC7.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 8) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC8.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 9) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC9.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 10) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC10.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat ==11) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC11.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 12) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC12.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 13) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC13.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 14) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC14.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 15) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC15.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 16) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC16.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 17) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC17.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 18) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC18.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 19) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC19.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 20) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC20.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 21) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC21.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 22) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC22.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 23) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC23.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 24) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC24.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 25) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC25.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 26) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC26.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 27) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC27.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 28) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC28.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 29) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC29.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 30) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC30.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 31) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC31.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
		...
		if (sat == 32) {
			FILE * fp = fopen("pos_position_20180127_MP_TEC32.txt", "a");                                    //打开写入文件
			if (fp == NULL)
			{
				perror("fopen");
			}
...
//------------------------clock repair--------------------------
static void clkRepair(rtk_t *rtk, obsd_t *obs, int n, const nav_t *nav)
{
	int i, sat, validGps, cjGps;
	int bObserved[MAXPRNGPS];
	double S1 = 0.0, S2 = 0.0, d1, d2, d3, d4, ddd1, ddd2;
	double lam1;
	double lam2;
	double M1, M11;
	double M2, M22;
	...
	{
		sat = obs[i].sat;
		lam1 = nav->lam[sat - 1][0];
		lam2 = nav->lam[sat - 1][1];
		...
		double deltp1 = obs[i].P[0] - rtk->ssat[obs[i].sat - 1].obs0[sat - 1][0];
		double deltp2 = obs[i].P[1] - rtk->ssat[obs[i].sat - 1].obs0[sat - 1][1];
		double deltl1 = (obs[i].L[0] - rtk->ssat[obs[i].sat - 1].obs0[sat - 1][2])* lam1;
		double deltl2 = (obs[i].L[1] - rtk->ssat[obs[i].sat - 1].obs0[sat - 1][3])* lam2;

		if (fabs(deltp1 - deltl1) > 290000)
		{
			S1 += deltp1 - deltl1;
			S2 += deltp2 - deltl2;
			cjGps++;
	...
	M1 == 0.0;   //flag for clock jump
	M11 = 0.0;
	M1 = d1 / CLIGHT * 1000.0;
	M11 = M1 - myRound(M1);//小数部分

	M2 == 0.0;   //flag for clock jump
	M22 = 0.0;
	M2 = d1 / CLIGHT * 1000.0;
	M22 = M2 - myRound(M2);
	...
	if (fabs(M11) < 2.5E-2)
	{
		rtk->ssat[obs[i].sat - 1].clkJump = myRound(M1);
		rtk->ssat[obs[i].sat - 1].clkJump2 = myRound(M2);
		//sprintf(PPP_Glo, "*** WARNING: clock jump=%d(ms)\n", PPP_Glo.clkJump);
	...
	for (i = 0; i < n; i++)
	{
		...
		rtk->ssat[obs[i].sat - 1].obs0[sat - 1][0] = obs[i].P[0];
		rtk->ssat[obs[i].sat - 1].obs0[sat - 1][1] = obs[i].P[1];
		rtk->ssat[obs[i].sat - 1].obs0[sat - 1][2] = obs[i].L[0];
		rtk->ssat[obs[i].sat - 1].obs0[sat - 1][3] = obs[i].L[1];


		ddd1 = rtk->ssat[obs[i].sat - 1].clkJump*CLIGHT / 1000.0;
		ddd2 = rtk->ssat[obs[i].sat - 1].clkJump2*CLIGHT / 1000.0;
	...
	for (i = 0; i < MAXPRNGPS; i++) {
		if (bObserved[i] == 0)
			rtk->ssat[obs[i].sat - 1].obs0[i][0] = rtk->ssat[obs[i].sat - 1].obs0[i][1] =
			rtk->ssat[obs[i].sat - 1].obs0[i][2] = rtk->ssat[obs[i].sat - 1].obs0[i][3] = 0.0;
	}
...
static void obsScan_PPP(const prcopt_t *popt, obsd_t *obs, const int nobs, int *nValid)
{
	int i, n, f2, sat, sys;
	...
	for (i = n = 0; i<nobs&&i<MAXOBS; i++) {
		...
		sys = satsys(sat, NULL);
		f2 = (sys&(SYS_GAL | SYS_SBS | SYS_CMP)) ? 2 : 1; /* L1/L2 or L1/L5 */
...
extern int rtkpos(rtk_t *rtk, obsd_t *obs, int n, const nav_t *nav)
{
    ...
    if (!spp(obs,nu,nav,&rtk->opt,&rtk->sol,NULL,rtk->ssat,msg)) 
	{
		if (outFp[OFILE_DEBUG])
		fprintf(outFp[OFILE_DEBUG],"point pos error\n");
...
//	return 0;
	...
	//----------------------------------------------计算多路径--------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------
	//cal_MP(rtk, obs, nu, nav);

	nn = n;
	obsScan_PPP(opt, obs, nu, &nn);

	if (nn <= 4) {
		if (outFp[OFILE_DEBUG])
		fprintf(outFp[OFILE_DEBUG], "*** WARNING: There are only %d satellites observed, skip PPP!\n", nn);
		return 0;
	}

	//add clock jump repair-------------------------------------------------------------------------------------
	clkRepair(rtk,obs, nn,nav);

	//------------------------------------------------------------------------------
    /* precise point positioning */
    if (opt->mode>=PMODE_PPP_KINEMA) {
        pppos(rtk,obs,nn,nav);
        outsolstat(rtk);
        return 1;
    }
