...
static double varerr(const prcopt_t *opt, double el, int sys)
{
	...
	//fact = sys == SYS_GLO ? PPP_Glo.prcOpt_Ex.errRatioGLO : (sys == SYS_CMP ? PPP_Glo.prcOpt_Ex.errRatioBDS : (sys == SYS_GAL ? PPP_Glo.prcOpt_Ex.errRatioGAL : (sys == SYS_QZS ? PPP_Glo.prcOpt_Ex.errRatioQZS : opt->err[0])));
	fact = 100.0;
	...
	double *v, double *H, double *var, double *azel, int *vsat, double *resp, int *nx, int *bDeleted)
{
	...
	*nx = NX_SPP;
	...
	for (i = 0; i<n&&i<MAXOBS; i++) {
		...
		if (!ionocorr(obs[i].time, nav, sys, pos, azel + i * 2, iter>0 ? opt->ionoopt : IONOOPT_BRDC, &dion, &vion)) continue;
		/* GPS-L1 -> L1/B1 */
	...
	const int *svh, const nav_t *nav, const prcopt_t *opt, sol_t *sol, double *azel, int *vsat, double *resp, char *msg)
{
	...
	for (i = 0; i<MAXITR; i++)
	{
		...
		if (d0<1E-4) {
			...
			sol->dtr[1] = x[5] / CLIGHT; /* bds-gps time offset (s) */
			sol->dtr[2] = x[4] / CLIGHT; /* glo-gps time offset (s) */
			...
			for (i = 0; i<3; i++)
				denu[i] = dxyz[i] = sol->rr[i] - ture_xyz[i];
			ecef2enu(pos, dxyz, denu);
			ecef2pos(sol->rr, pos);
			//printf( "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%04d%s%9.2f%s%14.7f%s%14.7f%s%14.7f%s%7.4f%s%7.4f%s%7.4f%s%7.4f%s%02d%s%4.2f\n",
				//(int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep, week, sep, sow, sep, sol->rr[0], sep, sol->rr[1], sep, sol->rr[2], sep, denu[0], sep, denu[1], sep, denu[2], sep, norm(denu, 3), sep, n, sep, sol->dop[1]);
		
			fprintf(outFp[OFILE_SPPPOS], "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%04d%s%9.2f%s%14.7f%s%14.7f%s%14.7f%s%7.4f%s%7.4f%s%7.4f%s%7.4f%s%02d%s%4.2f\n",
				(int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep, week, sep, sow, sep, sol->rr[0], sep, sol->rr[1], sep, sol->rr[2], sep, denu[0], sep, denu[1], sep, denu[2], sep, norm(denu, 3), sep, n, sep, sol->dop[1]);
			break;
		}
	}
	...
	const nav_t *nav, const prcopt_t *opt, sol_t *sol, double *azel, int *vsat, double *resp, char *msg)
{
	...
	for (nb = 0; nb <= 3; nb++) {
		...
		for (i = stat = 0; i<n; i++) {
			for (j = 0; j<nobs; j++)
				bDeleted[obs[j].sat - 1] = 1;
			for (j = i * nb; j<i*nb + nb; j++)
				bDeleted[obs[it[j] - 1].sat - 1] = 0;
	...
	sol_t *sol, double *azel, ssat_t *ssat, char *msg)
{
	...
	char *sep = " ";
	int week;
	double sow, ep[6], pos[3];
	time2epoch(obs[0].time, ep);
	sow = time2gpst(obs[0].time, &week);
	if (sow >= 378113&& sow <= 378200)return 0;
	//	printf("ok");
	...
	if (ssat) {
		...
		for (i = 0; i<NSYS; i++) sol->ns = 0;
		for (i = 0; i<NSYS; i++) sol->nsats[i] = 0;
		...
		for (i = 0; i<n; i++) {
			...
			if (sys == SYS_GPS) sol->nsats[0]++;
			else if (sys == SYS_GLO) sol->nsats[1]++;
			else if (sys == SYS_CMP) sol->nsats[2]++;
			else if (sys == SYS_GAL) sol->nsats[3]++;
			else if (sys == SYS_QZS) sol->nsats[4]++;
			else {
