...
static int decode_type1019(rtcm_t *rtcm)
{
    ...
    if (i+476<=rtcm->len*8) {
        ...
        printf("rtcm3 1019 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (prn>=40) {
        sys=SYS_SBS; prn+=80;
    }
	//printf("decode_type1019: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    
    if (rtcm->outtype) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
		printf(" prn=%2d iode=%3d iodc=%3d week=%d toe=%6.0f toc=%6.0f svh=%02X",
                prn,eph.iode,eph.iodc,week,eph.toes,toc,eph.svh);
    }
    if (!(sat=satno(sys,prn))) {
		printf("rtcm3 1019 satellite number error: prn=%d\n",prn);
        return -1;
    }
    eph.sat=sat;
...
static int decode_type1047(rtcm_t *rtcm)
{
	...
	double toc, sqrtA;
	char *msg;
	int i = 24, prn, sat, week, sys = SYS_CMP;
	...
	if (i + 476 <= rtcm->len * 8) {
		...
		eph.sva = getbitu(rtcm->buff, i, 4);              i += 4;
		// eph.code  =getbitu(rtcm->buff,i, 2);              i+= 2;//删除
		eph.idot = getbits(rtcm->buff, i, 14)*P2_43*SC2RAD; i += 14;
		...
		eph.omg = getbits(rtcm->buff, i, 32)*P2_31*SC2RAD; i += 32;
		eph.OMGd = getbits(rtcm->buff, i, 24)*P2_43*SC2RAD; i += 24;
		eph.tgd[0] = getbits(rtcm->buff, i, 10)*(0.1E-9);        i += 10;//
		eph.tgd[1] = getbits(rtcm->buff, i, 10)*(0.1E-9);        i += 10;//增加
		eph.svh = getbitu(rtcm->buff, i, 1);              i += 1;
		//eph.flag  =getbitu(rtcm->buff,i, 1);              i+= 1;//删除
		//eph.fit   =getbitu(rtcm->buff,i, 1)?0.0:4.0; /* 0:4hr,1:>4hr *///删除
		eph.fit = i;
	}
	else {
		trace(2, "rtcm3 1047 length error: len=%d\n", rtcm->len);
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
	...
	//-----------------------------------
	gtime_t time0 = {1242364607.0,0.0};
    //--------------------------------------
    ...
    switch (type) {
     ...
     //   case 1047: ret=decode_type1047(rtcm); break; /* beidou ephemeris (tentative mt) */
		case 1046:ret = decode_type1047(rtcm); break; /* beidou ephemeris (tentative mt) */
